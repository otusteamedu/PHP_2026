...
const results = $("#results");
const resultsBody = $("#results-table tbody");
const reportEmailInput = $("#report_email");
...
function sendValidation(emails, reportEmail) {
    if (!emails.length) {
        clearClientError();
        showClientError(
            "Не заполнен список emails для проверки"
        );
        results.hide();
        return;
    }
    ...
    if (!reportEmail.length) {
        clearClientError();
        showClientError(
            "Отсутствует электронный адрес для направления отчета"
        );
        results.hide();
        return;
    }
...
$("#check-btn").on("click", function () {
    const emails = normalizeInput(textarea.val());
    const reportEmail = reportEmailInput.val().trim();
    sendValidation(emails, reportEmail);
});
